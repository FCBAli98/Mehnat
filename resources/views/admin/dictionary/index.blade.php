  ...
  <div class="pull-right">
    <div class="btn-group group-control">
      <?php foreach ($languages as $lang => $locale): ?>
        ...
        <?php endif ?>
      <?php endforeach ?>
      <a href="{{route('admin.dictionary.create')}}" class="btn btn-primary btn-sm">{{ __('main.add_new') }}</a>
    </div>
  </div>
</div>
<br>
...
<table cellpadding="0" cellspacing="0" class="table table-bordered table-striped table-condensed">
  <thead>
    ...
    <tr id="searchRow">
      ...
      <th></th>
      <th>
          <?= Form::select('search[enabled]', [
              '' => 'Выберите',
              1 => 'Опубликовано',
              0 => 'Не активен'], isset(app('request')->input('search')['enabled'])?app('request')->input('search')['enabled']:null, ['class' => 'form-control', 'form' => 'searchForm']) ?>
      </th>
      <th></th>
      <th></th>
      <th></th>
    ...
    <?php if ($models): ?>
      <?php $i = $models->currentPage(); foreach ($models as $item): ?>
        <tr>
          ...
          <td>{{$item->status == 1 ? 'Актив' : 'Неактив'}}</td>
          <td>{{$item->created_at}}</td>
          <td>{{$item->updated_at}}</td>
          <td>
            <a href="{{route('admin.dictionary.show',['id' => $item->id])}}" class="btn btn-default btn-sm"><span class="fa fa-eye"></span></a>
            <a href="{{route('admin.dictionary.edit',['id' => $item->id])}}" class="btn btn-default btn-sm"><span class="fa fa-pencil"></span></a>
            <form action="{{route('admin.dictionary.destroy',['id' => $item->id])}}" method="post" style="display: none;" id='deleteItem-{{$item->id}}'>
              @csrf
