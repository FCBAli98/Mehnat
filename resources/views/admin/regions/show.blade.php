...
@section('title', 'Регионы - Просмотр')
...
<div class="clearfix">
    <div class="pull-left">
        <div class="btn-group group-control">
          ...
          <a href="{{ route('admin.regions.edit', ['id' => $model->id]) }}" class="btn btn-default">{{__('main.Edit')}}</a>
...
<table class="table table-bordered table-striped table-condensed">
  <tbody>
              ...
              <?php foreach ($model->langsCheck as $lang => $locale): ?>
                <?php if (isset($locale['exists'])): ?>
                  <tr class="success">
                    ...
                    <td>
                      <form action="{{route('admin.regions.destroyTranslate',['id' => $model->id, 'lang' => $lang])}}" method="post">
                        @csrf
                ...
                <?php else: ?>
                  <tr>
                    ...
                    <td>
                      <a href="{{route('admin.regions.edit',['id' => $model->id, 'lang' => $lang])}}" class="btn btn-primary btn-xs">Добавить перевод</a>
                    </td>
                  </tr>
                <?php endif ?>
              <?php endforeach ?>
      ...
      <tr>
        <th scope="row">Legal entity count</th>
        <td>{{ $model->legal_entity_count }}</td>
      </tr>
      <tr>
        <th scope="row">Physical entity count</th>
        <td>{{ $model->male_count + $model->female_count }}</td>
      </tr>
      <tr>
        <th scope="row">Male count</th>
        <td>{{ $model->male_count }}</td>
      </tr>
      <tr>
        <th scope="row">Female count</th>
        <td>{{ $model->female_count }}</td>
      </tr>
      <tr>
...
<div class="clearfix">
  <div class="pull-left">
    <form action="{{route('admin.regions.destroy',['id' => $model->id])}}" method="post" style="display: none;" id='deleteItem'>
        @csrf
        <input name="_method" type="hidden" value="DELETE">
      </form>
