...
<div class="clearfix">
    <div class="pull-left">
        <div class="btn-group group-control">
          ...
          <a href="{{ route('admin.pages.edit', ['id' => $model->id]) }}" class="btn btn-default">{{__('main.Edit')}}</a>
...
<table class="table table-bordered table-striped table-condensed">
  <tbody>
      ...
      <tr>
          ...
          <td>
            <?php if ($model->url): ?>
              <a href="{{route('pages.show', ['name' => $model->url], false)}}" target="_blank">{{route('pages.show', ['name' => $model->url], false)}}</a>
            <?php endif ?>
          </td>
      </tr>
      <tr>
              ...
              <?php foreach ($model->langsCheck as $lang => $locale): ?>
                <?php if (isset($locale['exists'])): ?>
                  <tr class="success">
                    ...
                    <td>
                      <form action="{{route('admin.pages.destroyTranslate',['id' => $model->id, 'lang' => $lang])}}" method="post">
                        @csrf
                ...
                <?php else: ?>
                  <tr>
                    ...
                    <td>
                      <a href="{{route('admin.pages.edit',['id' => $model->id, 'lang' => $lang])}}" class="btn btn-primary btn-xs">Добавить перевод</a>
                    </td>
                  </tr>
                <?php endif ?>
              <?php endforeach ?>
...
<div class="clearfix">
  <div class="pull-left">
    <form action="{{route('admin.pages.destroy',['id' => $model->id])}}" method="post" style="display: none;" id='deleteItem'>
        @csrf
        <input name="_method" type="hidden" value="DELETE">
      </form>
