...
@section('title', 'Категории документов - Просмотр')
...
<div class="clearfix">
    <div class="pull-left">
        <div class="btn-group group-control">
          ...
          <a href="{{ route('admin.document-categories.edit', ['id' => $model->id]) }}" class="btn btn-default">{{__('main.Edit')}}</a>
              ...
              <?php foreach ($model->langsCheck as $lang => $locale): ?>
                <?php if (isset($locale['exists'])): ?>
                  <tr class="success">
                    ...
                    <td>
                      <form action="{{route('admin.document-categories.destroyTranslate',['id' => $model->id, 'lang' => $lang])}}" method="post">
                        @csrf
                ...
                <?php else: ?>
                  <tr>
                    ...
                    <td>
                      <a href="{{route('admin.document-categories.edit',['id' => $model->id, 'lang' => $lang])}}" class="btn btn-primary btn-xs">Добавить перевод</a>
                    </td>
                  </tr>
                <?php endif ?>
              <?php endforeach ?>
...
<div class="clearfix">
  <div class="pull-left">
    <form action="{{route('admin.document-categories.destroy',['id' => $model->id])}}" method="post" style="display: none;" id='deleteItem'>
        @csrf
        <input name="_method" type="hidden" value="DELETE">
      </form>
