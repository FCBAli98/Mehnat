...
@section('title', 'Слайдеры - Просмотр')
...
<div class="clearfix">
    <div class="pull-left">
        <div class="btn-group group-control">
          <a href="{{ route('admin.sliders.index') }}" class="btn btn-default">{{__('main.List')}}</a>
          <a href="{{ route('admin.sliders.edit', ['id' => $model->id]) }}" class="btn btn-default">{{__('main.Edit')}}</a>
          <a href="{{ route('admin.sliders.create') }}" class="btn btn-default">{{__('main.add_new')}}</a>
        </div>
    </div>
              ...
              <?php foreach ($model->langsCheck as $lang => $locale): ?>
                <?php if (isset($locale['exists'])): ?>
                  <tr class="success">
                    ...
                    <td>
                      <form action="{{route('admin.sliders.destroyTranslate',['id' => $model->id, 'lang' => $lang])}}" method="post">
                        @csrf
                ...
                <?php else: ?>
                  <tr>
                    ...
                    <td>
                      <a href="{{route('admin.sliders.edit',['id' => $model->id, 'lang' => $lang])}}" class="btn btn-primary btn-xs">Добавить перевод</a>
                    </td>
                  </tr>
                <?php endif ?>
              <?php endforeach ?>
...
<div class="clearfix">
  <div class="pull-left">
    <form action="{{route('admin.sliders.destroy',['id' => $model->id])}}" method="post" style="display: none;" id='deleteItem'>
        @csrf
        <input name="_method" type="hidden" value="DELETE">
      </form>
